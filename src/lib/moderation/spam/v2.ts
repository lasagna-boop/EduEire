...
// Calibrated for safety-oriented moderation: prioritize catching true spam.
...
export function checkSpamV2(text: string): SpamResultV2 {
  ...
  const keyword = SPAM_KEYWORDS_V2.test(text);
  const contactHint = CONTACT_HINT_V2.test(text);

  if (urls >= 3) matches.push("spam_links");
  if (uppercaseRatio(text) > 0.68 && text.length >= 18) matches.push("spam_caps");
  ...
  if (OBFUSCATED_KEYWORD_V2.test(text)) matches.push("spam_obfuscated_keyword");
  if (contactHint && (urls >= 1 || keyword)) matches.push("spam_contact_combo");
  if (urls >= 2 && keyword) matches.push("spam_url_keyword_combo");
  if (punctuationRatio(text) >= 0.2 && text.length >= 24) matches.push("spam_symbol_noise");
